...
{
    //straight_update
    this->straight_data.stack = hypot((this->getPosition("x") - this->straight_data.data[0]),
                                      (this->getPosition("y") - this->straight_data.data[1]));
    //anglar_update
    double a_delta;
...
double T_Move::exeDistance(double targetAmount, double max_v, double point)
{
    ...
    //printf("%f\n", section);
    //定速制御
    if (abs(targetAmount) - section > point)
        return this->calcVelocityStraight(ACCELERATION, this->sign(targetAmount) * max_v);
    //加速＆減速制御
...
double T_Move::exeAngle(double targetAmount, double max_v, double point)
{
    ...
    if (abs(targetAmount) - section > abs(point))
        return this->calcVelocityTurn(ANGLE_ACCELERATION, this->sign(targetAmount) * max_v);
